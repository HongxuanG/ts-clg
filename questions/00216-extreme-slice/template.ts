 ...
 * 减一
 */
type SubN<A extends number, B extends number> = CreateArray<`${A}`> extends [
  ...infer Rest,
  ...CreateArray<`${B}`>
]
  ? Rest['length']
...
> = equal<End, Start> extends true
  ...
  ? `${End}` extends `-${infer PositiveEnd}`
    ? Slice<
        Arr,
        AddOne<SubN<Arr['length'], StringToNumber<PositiveStart>>>,
        SubN<Arr['length'], StringToNumber<PositiveEnd>>,
        [...ResultArr, Arr[Start]]
      >
    : Slice<
        Arr,
